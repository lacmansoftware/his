...
import type { AppointListData } from './types'

const request = useAxios()

export const getTableListApi = async (params: any): Promise<IResponse> => {
  const res = await request.get({ url: '/member/appointment/wait/cancel/list', params })
  return res && res.data
}
...
export const getTableDetApi = async (id: string): Promise<IResponse<AppointListData>> => {
