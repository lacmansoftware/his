...
export const getTableListApi = async (params: any): Promise<IResponse> => {
  const res = await request.get({ url: '/member/visit/record/recordLogList', params })
  return res && res.data
}

export const getRecordListApi = async (params: any): Promise<IResponse> => {
  const res = await request.get({ url: '/member/visit/record/recordList', params })
  return res && res.data
}
