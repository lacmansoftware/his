...
import {
  ...
  delTableListApi,
  saveTableApi
} from '@/api/pharmacyCore/product_core/stockin'
import { NotChargedTableData } from '@/api/pharmacyCore/product_core/stockin/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '請貨門店', field: 'hospitalName', width: '100px' },
  { label: '入庫單編號', field: 'id', width: '120px' },
  { label: '採購單編號', field: 'purchaseId', width: '120px' },
  {
    label: '狀態',
    field: 'status',
    width: '55px',
  ...
  { label: '供應商', field: 'supplierName', width: '100px' },
  { label: '進貨總價', field: 'totalPurchasePrice', width: '60px' },
  { label: '銷售總價', field: 'salePrice', width: '60px' },
  { label: '創建人', field: 'createUser', width: '55px' },
  { label: '創建日期', field: 'createTime', width: '80px' },
  { label: '審核人', field: 'approver', width: '55px' },
  { label: '審核時間', field: 'approveTime', width: '100px' },

  // Search Schema
  genSearchSchema('input', 'id', '入庫單編號', { placeholder: '入庫單編號' }),
  genSearchSchema('input', 'purchaseId', '採購單編號', { placeholder: '採購單編號' }),
  ...
  genSearchSchema('apiSelect', 'supplierId', '供應商', {
    placeholder: '請選擇',
    api: async () => {
      return await getInOptionFormat('product/supplier/list', 'id', 'name')
    }
  }),
