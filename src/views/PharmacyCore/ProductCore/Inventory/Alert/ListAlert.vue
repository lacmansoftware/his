...
import {
  ...
  delTableListApi,
  saveTableApi
} from '@/api/pharmacyCore/product_core/inventory/alert'
import { NotChargedTableData } from '@/api/pharmacyCore/product_core/inventory/alert/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    width: '60px',
    formatter: function (row) {
      return row.sysProduct.unitValue
    }
  },
  {
    label: '所屬類別',
    ...
    width: '70px',
    formatter: function (row) {
      return inDict(row.sysProduct.type, 'product.type')
    }
  },
  { label: '預警上限', field: 'maxAlertQty', width: '90px' },
  { label: '預警下限', field: 'minAlertQty', width: '90px' },
  { label: '有效期下限', field: 'expireMinAlert', width: '90px' },
  {
    label: '實際庫存',
    field: 'totalQty',
    width: '100px',
    formatter: function (row) {
      return row.productInventory.totalQty
    }
  },
  {
    label: '可用庫存',
    ...
    width: '100px',
    formatter: function (row) {
      return row.productInventory.currentQty
    }
  },
  { label: '創建日期', field: 'createTime', width: '120px' },
  { label: '更新日期', field: 'modifyTime', width: '120px' },

  // Search Schema
  genSearchSchema('input', 'productName', '商品', { placeholder: '商品名稱' }),
