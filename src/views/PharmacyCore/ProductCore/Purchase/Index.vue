...
import {
  ...
  delTableListApi,
  saveTableApi
} from '@/api/pharmacyCore/product_core/purchase'
import { NotChargedTableData } from '@/api/pharmacyCore/product_core/purchase/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '請貨單號', field: 'applyOrderId', width: '120px' },
  { label: '門店', field: 'hospitalName', width: '110px' },
  { label: '供應商', field: 'supplierName', width: '125px' },
  {
    label: '狀態',
    field: 'status',
    width: '60px',
  ...
  genSearchSchema('input', 'applyOrderId', '請貨單號', { placeholder: '請貨單號' }),
  ...
  genSearchSchema('input', 'supplierName', '供應商', { placeholder: '供應商' }),
    ...
    >
      <template #action="{ row }">
        <div v-if="row.status === 'P'">
          ...
          <ElLink type="primary" @click="settlement(row)" class="mr-5px">撤銷採購計劃</ElLink>
        </div>
        <div v-else>
          <ElLink type="primary" @click="settlement(row)" class="mr-5px">查看明細</ElLink>
        </div>
