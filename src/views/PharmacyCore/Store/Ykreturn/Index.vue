...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/pharmacyCore/store/ykreturn'
import { NotChargedTableData } from '@/api/pharmacyCore/store/ykreturn/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: function (row) {
      return inDict(row.status, 'pharmacy.ykReturnStatus')
    }
  },
  { label: '創建日期', field: 'createTime' },
  { label: '創建人', field: 'createUser' },
  { label: '審核人', field: 'approveUser' },
  { label: '審核時間', field: 'approveTime' },

  // Search Schema
  genSearchSchema('input', 'id', '退藥單號', { placeholder: '退藥單號' }),
  genSearchSchema('autocomplete', 'drugId', '藥品', {
    url: '/drug/info/fuzzyQuery',
    itemValue: 'searchText',
    itemLink: 'id',
    placeholder: '藥品編碼/名稱/拼音'
  }),
  genSearchSchema('apiSelect', 'pharmacyId', '藥房', {
    placeholder: '藥房',
    api: async () => {
      return await getInOptionFormat('pharmacy/info', 'undefined', 'name')
    }
  }),
