...
import {
  ...
  delTableListApi,
  saveTableApi
} from '@/api/pharmacyCore/inventory/batchNo'
import { NotChargedTableData } from '@/api/pharmacyCore/inventory/batchNo/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '藥房', field: 'pharmacyInfoName' },
  { label: '生產批號', field: 'produceBatchNum' },
  { label: '藥品名稱', field: 'drugName' },
  { label: '批次號', field: 'batchId' },
  { label: '供應商', field: 'supplierName' },
  { label: '創建時間', field: 'createTime' },

  // Search Schema
  genSearchSchema('input', 'drugName', '藥品名稱/拼音', { placeholder: '藥品名稱' }),
  genSearchSchema('input', 'produceBatchNum', '生產批號', { placeholder: '生產批號' }),
...
<template>
  <ContentWrap>
    ...
    >
      <template #action="{ row }">
        <ElLink type="primary" @click="settlement(row)" class="mr-5px">修改生產批號</ElLink>
      </template>
    </Table>
  </ContentWrap>
