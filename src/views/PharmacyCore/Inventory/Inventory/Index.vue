...
import {
  ...
  delTableListApi,
  saveTableApi
} from '@/api/pharmacyCore/inventory/inventory'
import { NotChargedTableData } from '@/api/pharmacyCore/inventory/inventory/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '批次號', field: 'batchId', width: '100px' },
  {
    ...
    formatter: function (row) {
      return inDict(row.status, 'inventoryStatus1')
    }
  },
  { label: '總量', field: 'totalQty', width: '60px' },
  { label: '當前量', field: 'currentQty', width: '60px' },
  { label: '零售價', width: '60px', field: 'sellingUnitPrice' },
  { label: '進貨價', field: 'purchaseUnitPrice', width: '60px' },
  { label: '供應商', field: 'supplier', width: '90px' },
  { label: '類別', field: 'drugType', width: '60px' },
  ...
  genSearchSchema('apiSelect', 'pharmacyId', '藥房', {
    placeholder: '請選擇',
    api: async () => {
      return await getInOptionFormat('pharmacy/info/list/withoutyanxuan', 'id', 'name')
    }
  }),
  genSearchSchema('sourceSelect', 'status', '狀態', {
    placeholder: '請選擇',
    options: dict.inventoryStatus1 as any
  }),
  genSearchSchema('checkbox', 'isMerge', '', { options: dict.merge, value: [] })
])
    ...
    >
      <template #action="{ row }">
        <div v-if="row.isMerge === 'Y'">
          <div v-if="row.pharmacyId">
            <ElLink type="primary" @click="settlement(row)" class="mr-5px">調整價格</ElLink>
            <ElLink
              v-if="row.isChangePrice === 'Y'"
              type="primary"
