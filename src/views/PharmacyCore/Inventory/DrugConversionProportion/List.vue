...
import {
  ...
  delTableListApi,
  saveTableApi
} from '@/api/pharmacyCore/inventory/drugConversionProportion'
import { NotChargedTableData } from '@/api/pharmacyCore/inventory/drugConversionProportion/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '修改時間', field: 'modifyTime' },

  // Search Schema
  genSearchSchema('input', 'drugName', '藥品名稱', { placeholder: '藥品名稱' }),
  genSearchSchema('apiSelect', 'pharmacyId', '藥房', {
    placeholder: '藥房',
    api: async () => {
      return await getInOptionFormat('pharmacy/info?dosageforms=k', 'id', 'name')
    }
  })
])
...
const tableRowClassName = ({ row, rowIndex }: { row: any; rowIndex: number }) => {
  // if (
  //   row.pharmacyInventory.totalQty < row.minAlertQty ||
  //   row.pharmacyInventory.totalQty > row.maxAlertQty
  // ) {
