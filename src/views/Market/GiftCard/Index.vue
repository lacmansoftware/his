...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/market/giftCard'
import { NotChargedTableData } from '@/api/market/giftCard/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: function (row) {
      return inDict(row.status, 'giftCardStatus')
    }
  },
  {
    label: '類型',
    ...
    width: '40px',
    formatter: function (row) {
      return inDict(row.type, 'giftCardType')
    }
  },
  { label: '名稱', field: 'name', width: '50px' },
  { label: '面額', field: 'amount', width: '50px' },
  { label: '卡密總數', field: 'number', width: '40px' },
  { label: '有效期', field: 'termTime', width: '80px' },
  { label: '創建人', field: 'createUser', width: '40px' },
  { label: '創建時間', field: 'createTime', width: '80px' },
  { label: '更新人', field: 'modifyUser', width: '40px' },
  { label: '更新時間', field: 'modifyTime', width: '80px' },

  // Search Schema
  genSearchSchema('input', 'name', '名稱', { placeholder: '名稱' }),
  genSearchSchema('input', 'voucherCode', '卡密', { placeholder: '卡密' }),
  genSearchSchema('sourceSelect', 'type', '類型', {
    placeholder: '類型',
    options: dict.giftCardType as any
  }),
  genSearchSchema('sourceSelect', 'status', '狀態', {
    placeholder: '狀態',
    options: dict.giftCardStatus as any
  })
])
...
<template>
  <ContentWrap>
    ...
    <div class="mb-10px ml-10px mt-[-32px]">
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">新增線下卡</ElButton>
    </div>
    ...
    >
      <template #action="{ row }">
        <div v-if="row.status === 'N'">
          ...
          <ElLink type="primary" @click="settlement(row)" class="mr-5px">開始製作</ElLink>
          <ElLink type="primary" @click="settlement(row)" class="mr-5px">刪除</ElLink>
        </div>
        <div v-else-if="row.status === 'Y'">
          <ElLink type="primary" @click="settlement(row)" class="mr-5px">卡密表</ElLink>
        </div>
      </template>
    </Table>
  </ContentWrap>
