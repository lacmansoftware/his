...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/market/product/activity'
import { NotChargedTableData } from '@/api/market/product/activity/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '名稱', field: 'activityName' },
  {
    label: '類型',
    field: 'type',
    formatter: function (row) {
      return inDict(row.type, 'product.activityType')
    }
  },
  {
    label: '促銷途徑',
    field: 'promotionWay',
    formatter: function (row) {
      return inDict(row.promotionWay, 'product.promotionWay')
    }
  },
  { label: '參与門店', field: 'hospitalName' },
  { label: '開始時間', field: 'startTime' },
  { label: '結束時間', field: 'endTime' },
  {
    label: '狀態',
    field: 'status',
    formatter: function (row) {
      return inDict(row.status, 'product.activityStatus')
    }
  },
  { label: '創建時間', field: 'createTime' },
  // Search Schema
  genSearchSchema('input', 'activityName', '活動名稱', { placeholder: '活動名稱' }),
  genSearchSchema('sourceSelect', 'type', '活動類型', {
    placeholder: '活動類型',
    options: dict.product.activityType as any
  }),
...
<template>
  <ContentWrap>
    ...
    <div class="mb-10px ml-10px mt-[-32px]">
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">新增產品促銷</ElButton>
    </div>
