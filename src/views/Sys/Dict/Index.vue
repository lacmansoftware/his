...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/sys/dict'
import { NotChargedTableData } from '@/api/sys/dict/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '名稱', field: 'name' },
  { label: '包含內容項', field: 'childSize' },
  { label: '類型編碼', field: 'type' },
...
<template>
  <ContentWrap>
    ...
    >
      <template #action="{ row }">
        <ElLink type="primary" @click="settlement(row)" class="mr-5px">管理</ElLink>
      </template>
    </Table>
  </ContentWrap>
