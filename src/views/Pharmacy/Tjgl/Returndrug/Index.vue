...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/pharmacy/tjgl/returndrug'
import { NotChargedTableData } from '@/api/pharmacy/tjgl/returndrug/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '患者', field: 'patientName', width: '60px' },
  { label: '手機', field: 'patientMobile', width: '80px' },
  ...
  { label: '劑型', field: 'drugState', width: '70px' },
  { label: '總劑數', field: 'totalNum', width: '50px' },
  {
    label: '配送方式',
    field: 'expressName',
    width: '70px',
    formatter: function (row) {
      const v = row.expressName
      let str = ''
      if (v !== '') {
        str = v
      }
      if (row.expressType === 'EXPRESS') {
        str = '快遞'
      }
      return str
    }
  },
  { label: '下單時間', field: 'createTime' },
  {
    label: '支付狀態',
    field: 'itemStatus',
    formatter: function (row) {
  ...
  { label: '操作人', field: 'returnUser' },
  { label: '操作時間', field: 'returnTime' },
  ...
  genSearchSchema('input', 'patientName', '患者信息', { placeholder: '患者姓名/手機/病案號' }),
  genSearchSchema('input', 'doctorName', '醫生', { placeholder: '醫生' }),
  genSearchSchema('sourceSelect', 'recipelStatus', '退葯狀態', {
    placeholder: '退葯狀態',
    options: [
      { value: 'SEND', label: '未退葯' },
      { value: 'RETURN', label: '已退葯' }
    ] as any
  }),
  ...
  <ContentWrap>
    ...
    >
      <template #action="{ row }">
        ...
        <ElLink
          v-if="row.returnDrugMark !== '1'"
          type="primary"
          @click="settlement(row)"
          class="mr-5px"
          >退葯</ElLink
        >
        <ElLink type="primary" @click="settlement(row)" class="mr-5px">日誌</ElLink>
      </template>
    </Table>
  </ContentWrap>
