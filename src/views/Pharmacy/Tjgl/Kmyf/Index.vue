...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/pharmacy/tjgl/kmyf'
import { NotChargedTableData } from '@/api/pharmacy/tjgl/kmyf/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: function (row) {
      return inDict(row.kmStatus, 'kmOrderStatus')
    }
  },
  { label: '康美訂單號', field: 'kmOrderId', width: '130px' },
  { label: '處方編號', field: 'recipelCode', width: '150px' },
  { label: '患者', field: 'patientName', width: '80px' },
  { label: '電話', field: 'patientMobile', width: '90px' },
  ...
  { label: '煎藥信息', field: 'decoType', width: '70px' },
  { label: '劑型', field: 'drugState', width: '70px' },
  { label: '下單時間', field: 'time', width: '130px' },
  { label: '審核結果', field: 'kmRemark' },
  {
    label: '退葯狀態',
  ...
  genSearchSchema('sourceSelect', 'decoNum', '煎藥信息', {
    placeholder: '煎藥信息',
    options: dict.decoNum as any
  })
])
...
<template>
  <ContentWrap>
    ...
    <div v-if="pageType !== 'approve'" class="mb-10px ml-10px mt-[-32px]">
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">全部發葯</ElButton>
    </div>
