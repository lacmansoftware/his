...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/pharmacy/inventory/check'
import { NotChargedTableData } from '@/api/pharmacy/inventory/check/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '盤庫單編號', field: 'id', width: '120px' },
  {
    label: '狀態',
    field: 'status',
    width: '60px',
  ...
  { label: '審核時間', field: 'approveTime', width: '120px' },
  { label: '審核人', field: 'approver', width: '60px' },
  {
    label: '盤庫類型',
    field: 'checkType',
    width: '60px',
  ...
  genSearchSchema('input', 'drugKeywords', '藥品', { placeholder: '藥品' }),
...
<template>
  <ContentWrap>
    ...
    <div v-if="pageType !== 'approve'" class="mb-10px ml-10px mt-[-32px]">
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">新建盤庫單</ElButton>
    </div>
