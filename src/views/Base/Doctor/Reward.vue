...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/base/doctor/reward'
import { NotChargedTableData } from '@/api/base/doctor/reward/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '職稱', field: 'titles' },
  { label: '區域', field: 'city' },
  { label: '經紀人', field: 'bdNames' },
  { label: '城市經理', field: 'bdCityNames' },
  { label: '本月已提現金額', field: 'currentMonthAmount' },
  { label: '累計已提現金額', field: 'totalAmount' },
  { label: '錢包金額', field: 'balance' },
  {
    label: '啟用狀態',
    field: 'doctorStatus',
    formatter: function (row) {
      return inDict(row.doctorStatus, 'doctorStatus2')
    }
  },
  {
    label: '認證狀態',
    field: 'authStatus',
    formatter: function (row) {
      return inDict(row.authStatus, 'doctorStatus')
    }
  },
  { label: '創建時間', field: 'createTime', width: '50px' },
  // Search Schema
  genSearchSchema('input', 'doctorName', '醫生姓名', { placeholder: null }),
  genSearchSchema('input', 'doctorMobile', '手機號', { placeholder: null }),
  genSearchSchema('sourceSelect', 'protocal', '是否同意協議', {
    placeholder: null,
    options: dict.doctor.protocal as any
  }),
  genSearchSchema('sourceSelect', 'autoSettle', '是否自動結算', {
    placeholder: null,
...
<template>
  <ContentWrap>
    ...
    <div class="mb-10px ml-10px mt-[-32px]">
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">下載導入模板</ElButton>
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">導入獎勵金額</ElButton>
      <ElButton type="primary" @click="AddAction" :icon="plusIcon">未提現分成</ElButton>
    </div>
    ...
    >
      <template #action="{ row }">
        <ElLink type="primary" @click="settlement(row)" class="mr-5px">增加獎勵金額</ElLink>
        <ElLink type="primary" @click="settlement(row)" class="mr-5px">交易明細</ElLink>
        <ElLink type="primary" @click="settlement(row)" class="mr-5px">分成預覽</ElLink>
      </template>
    </Table>
  </ContentWrap>
