...
import { getTableListApi, delTableListApi, saveTableApi } from '@/api/base/doctor/comments'
import { NotChargedTableData } from '@/api/base/doctor/comments/types'
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  { label: '客人姓名', field: 'patientName' },
  { label: '客人手機', field: 'patientMobile' },
  { label: '醫生', field: 'doctorName' },
  { label: '評論時間', field: 'createTime' },
  {
    label: '審核狀態',
    field: 'status',

    formatter: function (row) {
      if (row.status === 'REGIST') {
        return '已登記'
      }
  ...
  },
  {
    ...
    formatter: function (row) {
      ...
      if (row.status2 === 'REGIST') {
        return '已登記'
      }
      if (row.status2 === 'REJECT') {
        return '已駁回'
      }
      if (row.status2 === 'PASS') {
        return '已通過'
  ...
  { label: '治療效果', field: 'treatScore' },
  { label: '大夫服務', field: 'type' },
  { label: '就診服務', field: 'serviceType' },
  { label: '醫生印象', field: 'contentTags' },
  { label: '評價詳情', field: 'content' },
  {
    label: '上一次評價',
    field: 'recent',
    formatter: function (row) {
      if (!row.recent) {
        return '--'
      } else return '' + row.recent + ''
    }
  },

  // Search Schema
  genSearchSchema('autocomplete', 'doctorId', '選擇醫生：', {
    url: '/doctor/query',
  ...
  genSearchSchema('sourceSelect', 'status', '審核狀態：', {
    placeholder: '請選擇',
    options: dict.commonStatus as any
  }),
  genSearchSchema('input', 'nameOrMobile', '客人姓名/手機', { placeholder: null }),
  genSearchSchema('datePicker', 'startTime', '評論時間：', { placeholder: '評論時間' }),
  genSearchSchema('datePicker', 'endTime', '到', { placeholder: '評論時間' }),
  genSearchSchema('input', 'content', '評價詳情', { placeholder: '評價詳情' })
])
